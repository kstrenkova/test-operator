...
const (
	...
	VolumeNameTestOperatorLogs = "test-operator-logs"

	secretNameCombinedCABundle = "combined-ca-bundle"
	secretNameOpenstackConfig  = "openstack-config-secret"

	subPathCloudsYAML       = "clouds.yaml"
	subPathKubeconfigConfig = "config"
...
func CreateOpenstackConfigSecretVolume() corev1.Volume {
	...
	return corev1.Volume{
		Name: secretNameOpenstackConfig,
		VolumeSource: corev1.VolumeSource{
			Secret: &corev1.SecretVolumeSource{
				DefaultMode: &mode,
				SecretName:  secretNameOpenstackConfig,
			},
		},
	}
}
...
func CreateLogsPVCVolume(logsPVCName string) corev1.Volume {
	return corev1.Volume{
		Name: VolumeNameTestOperatorLogs,
		VolumeSource: corev1.VolumeSource{
	...
	kubeconfigVolume := corev1.Volume{
		...
		VolumeSource: corev1.VolumeSource{
			Secret: &corev1.SecretVolumeSource{
				...
				Items: []corev1.KeyToPath{
					{
						Key:  subPathKubeconfigConfig,
						Path: subPathKubeconfigConfig,
					},
				},
			},
		},
...
func CreateOpenstackConfigVolumeMount(mountPath string) corev1.VolumeMount {
	return CreateVolumeMountWithSubPath("openstack-config", mountPath, subPathCloudsYAML, true)
}
...
func CreateOpenstackConfigSecretVolumeMount(mountPath string) corev1.VolumeMount {
	return CreateVolumeMountWithSubPath(secretNameOpenstackConfig, mountPath, subPathSecureYAML, false)
}
