...
// ValidateNameLength checks if CR name exceeds DNS label length limit
func ValidateNameLength(allErrs field.ErrorList, name, kind string) field.ErrorList {
...
func CheckPrivilegedWarning(allWarn admission.Warnings, privileged bool, kind string) admission.Warnings {
	...
	if !privileged && kind == "Tobiko" {
		allWarn = append(allWarn, fmt.Sprintf(WarnPrivilegedModeOff, kind))
	}
	return allWarn
}

// CheckSELinuxWarning returns warning if privileged mode + workflow but no SELinux level
func CheckSELinuxWarning(allWarn admission.Warnings, privileged bool, workflow interface{}, seLinuxLevel, kind string) admission.Warnings {
	if privileged && reflect.ValueOf(workflow).Len() > 0 && len(seLinuxLevel) == 0 {
...
func ValidateDebugWorkflow(allErrs field.ErrorList, debug bool, workflow interface{}, kind string) field.ErrorList {
	if debug && reflect.ValueOf(workflow).Len() > 0 {
		allErrs = append(allErrs, &field.Error{
